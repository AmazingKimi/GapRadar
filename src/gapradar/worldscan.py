...
def _forced_gate(change_type: str, text: str) -> bool:
    ...
    if change_type == "price_shock":
        return bool(re.search(r"\b(price|pricing|fee|subscription|cost)\b", t) and re.search(r"\b(increase|hike|raise|raised|rising|change|changes|higher|up|charge|charging)\b", t)) or bool(re.search(r"\bfree tier\b.{0,80}\b(end|ending|remove|removed|cut|limit|limited)\b", t))

...
def render_dashboard(events: list[MarketEvent], output: Path) -> None:
    ...
    passed = sum(1 for event in events if build_dossier(event).verdict == "PASS")
...
<body><main><div class="eyebrow">Opportunity Intelligence · v0.5</div><h1>GapRadar</h1>
<p class="lede">Verified market changes become evidence-grounded opportunity dossiers. V0.5 explains what changed, who appears displaced, what substitutes exist, what remains unsolved, and whether the evidence says REVIEW, WATCH, PASS, or LIKELY SERVED.</p>
<section class="metrics"><div class="metric"><strong>{len(events)}</strong><span>verified events</span></div><div class="metric"><strong>{reviewed}</strong><span>review dossiers</span></div><div class="metric"><strong>{gaps}</strong><span>potential gaps</span></div><div class="metric"><strong>{passed}</strong><span>pass verdicts</span></div></section>
<section class="grid">{cards_html}</section><footer>Generated by GapRadar v0.5 · Dossiers summarize evidence; they do not invent market size or tell you to build.</footer></main></body></html>"""
